...
void connect_to_sr()
{
   ...
   // Connect to sysrepo
   rc = sr_connect("stats_tests.c", SR_CONN_DEFAULT, &sr_conn_link.conn);
   IF_SR_ERR_FAIL(rc)
...
{
   system("helpers/import_conf.sh -s nemea-test-1-startup-config-stats-1.xml");
   connect_to_sr();
   load_config();
   subscribe_xpath_stats("/nemea-test-1:nemea-supervisor/module-group/module"
...
void test_get_intable_module_stats(void **state)
{
   ...
   connect_to_sr();
   load_config();
   subscribe_xpath_stats("/nemea-test-1:nemea-supervisor/module-group/module/instance",
