...
static void config_change_cb_test_group_with_modules_created(void **state)
{
   ...
   VERBOSE(DEBUG, "Subscribing")
   rc = sr_module_change_subscribe(sr_conn_link.sess, "nemea-test-1", config_change_cb_wrapper, NULL, 0, SR_SUBSCR_DEFAULT | SR_SUBSCR_APPLY_ONLY, &sub);
...
{
   system("helpers/import_conf.sh -s nemea-test-1-startup-config-3.xml");
   ...
   // 4 already defined in yang/nemea-test-1-startup-config-1.xml
